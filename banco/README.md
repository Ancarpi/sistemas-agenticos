...
Nueve ficheros de este directorio no los escribe el extractor. Cuatro son el empaquetado y se editan a mano: `README.md`, `pyproject.toml`, `docker-compose.yml` y `conftest.py`. El quinto y el sexto también van a mano y son la excepción de `tests/`: ejercitan el árbol en vez de salir de sus páginas --- `tests/test_supresion.py`, los dos casos negativos que el red team dejó sobre la supresión del 34.6, y `tests/test_integracion.py`, el humo de integración que cruza la cola del 35.6, la memoria del 34.7, la supresión del 34.6 y el `decidir` del 37.2 con un solo sujeto ---. Dos son artefactos del libro traducidos a datos, también a mano: `contratos/triage_sepa.contract.yaml` (el `TRIAGE_CONTRACT` del 19.1, de dataclass a YAML) y `evals/capstone_gate.eval.yaml` (la tabla del 18.4, con el 246 que el 15.7 justifica en vez del 120 impreso). El noveno, `.env.example`, también sale del libro, solo que por otro script: lo genera `../tools/generar_env/generar_env.py` desde el bloque `.env` del 0.4, y ese script sale con código 1 y nombra la variable cuando algo de este árbol lee con `os.environ` una que el libro no declara.
...
| `tests/` (el árbol entero) | `uv run pytest tests/` | 16.7, 34.6, 37.2 |

Esa última fila es la que más letra pequeña tiene, así que va entera. `pytest` se ejecuta desde `banco/`, y necesita dos cosas que el árbol del Ejercicio 0.1 no da solo. Las dos están puestas: el `pythonpath` de `[tool.pytest.ini_options]` en `pyproject.toml`, porque `tests/test_art50.py` importa `cumplimiento` plano, igual que el libro, y aquí ese fichero vive en `src/core/` --- y `plataforma/` va en la misma lista, porque `tests/test_supresion.py` importa la supresión del 34.6 y esta llega a `trazas` igual de plano vía `memoria.py`, y `tests/test_integracion.py` importa por ahí el `runtime.py` del 37.2; y `conftest.py`, que carga el `.env`, porque `src/core/cumplimiento.py` abre su pool a nivel de módulo y sin `DATABASE_URL` el import muere antes de la primera aserción, pytest aborta la tanda entera y ejecuta cero tests. Si falta el `.env`, la cabecera de pytest lo dice en una línea.

Comprobado sobre este árbol: con `.env` puesto y Postgres arriba --- el `db/schema.sql` **y** el `src/core/memoria.sql` del 34.7 aplicados, porque la supresión que miden los tres últimos toca `banco.memoria` ---, `6 passed`. Con `.env` puesto y sin Postgres, `2 passed, 1 failed, 3 skipped`, y ese reparto es el resultado correcto: `tests/test_registro.py` mide la RLS del 16.6 contra la base de datos, y un test de RLS sin base de datos no mide nada. Falla, y no se salta. Lo que sale es `psycopg.OperationalError: connection failed`. Ese test se conecta con `DATABASE_URL_APP`, que es el rol `banco_app` del 16.6 y no el superusuario, por lo que dice la arruga de abajo. Los tres `skipped` son los dos casos de `tests/test_supresion.py` --- los que el red team dejó sobre la supresión del 34.6: que un guion bajo en el sujeto no actúe de comodín del `LIKE` y borre filas de OTRO cliente, y que una propuesta pendiente del sujeto se cierre firmada antes de purgarse, en vez de quedar vaciada y aprobable a ciegas --- y el humo de `tests/test_integracion.py`, y saltar es su resultado correcto: miden código de aplicación, no una promesa del schema, y sin base de datos no mienten ni en rojo ni en verde.
