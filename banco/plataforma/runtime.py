...
def decidir(ctx, llamada) -> str | None:
    ...
    if decision == "require_human":
        ...
        # consola del 35.4 lista con `pendientes` es
        # `banco.aprobaciones`: sin esta llamada nadie ve la
        # propuesta, nadie la decide y el hilo se queda parado. La
        # regla del 11.5 sigue en pie, porque lo que prohíbe por
...
# --- anadido del M37.2 (extraer_banco) ---
# Anadido del M37.2.
# runtime.py --- nadie importa un grafo: se pide por id. Ni
# `autorizar` (el 26.5) ni `encolar` (el 35.6) se escriben aquí.
import hashlib
