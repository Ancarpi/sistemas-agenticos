...
VOZ = """Eres el asistente telefónico del banco. Hablas
...
async def construir_cerebro():
    ...
    if len(dos) != len(PERMITIDAS):
        faltan = sorted(PERMITIDAS - {t.name for t in dos})
        raise RuntimeError(f"el core no expone {faltan}:"
                           " bloquear_tarjeta la añade el"
                           " Ejercicio 10.1(5)")
    return create_agent(
