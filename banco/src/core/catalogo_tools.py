...
def _fallos(m: dict) -> list:
    """Todos los defectos, y los dos últimos miran coherencia."""
...
def _cabe(m: dict, sujeto: dict, ctx: dict) -> bool:
    ...
    if not set(m["auth"]["scopes"]) <= set(sujeto["scopes"]):
        return False
    if sujeto["nivel_identidad"] < m["auth"].get("identity_level", 0):
        return False
...
def clase_y_peldano(nombre: str) -> tuple[str, str]:
    ...
    m = CATALOGO[nombre]
    return m["audit"].get("data_class", "internal"), m["risk_tier"]
