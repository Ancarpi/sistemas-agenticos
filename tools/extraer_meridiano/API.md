# tools/extraer_meridiano — API

Rebuilds `banco-meridiano/` from `libro.md`. The book is the source; the repo is
...
```
extraer_meridiano.py [--libro PATH] [--destino DIR] [--mapeo PATH]
extraer_meridiano.py --verificar        # checks anchors, writes nothing
extraer_meridiano.py --resincronizar    # relocates ranges by anchor, rewrites mapeo.yaml
```

Defaults assume the author's layout: `../../../fuente/libro.md` and
`../../banco-meridiano` relative to this directory.
...
- **Four modes.** `verbatim` creates the destination from its blocks concatenated
  ...
  embeds a block into `banco-meridiano/COSTURAS.md` instead of a code file.
...
- **Generated, therefore never hand-edited:** the whole `banco-meridiano/` tree
...
- It does not run, lint or import anything it writes. Which files actually run is
  in `banco-meridiano/README.md`, and that judgement is not extractable.
...
- None. It is a maintenance tool: the author runs it when the book changes, and
  `banco-meridiano/README.md` documents it for readers of the repo.
