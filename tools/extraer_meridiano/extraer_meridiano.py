#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""extraer_meridiano --- reconstruye banco-meridiano/ desde libro.md.
...
vez de escribir basura.
    ...
    python3 extraer_meridiano.py --libro ../../../fuente/libro.md
    python3 extraer_meridiano.py --verificar        # no escribe nada
    python3 extraer_meridiano.py --resincronizar    # reubica por ancla
...
DESTINO_POR_DEFECTO = RAIZ.parents[1] / "banco-meridiano"
...
def reubicar(lineas, ancla, ancla_fin, cerca_de, largo):
    ...
    for i in range(1, len(lineas) + 1):
        if lineas[i - 1].strip() != ancla:
            continue
        for j in range(i, min(i + techo, len(lineas)) + 1):
            if lineas[j - 1].strip() != ancla_fin:
                continue
...
def marcador(destino, modulo, nota, pendiente):
    ...
    return (f"\n\n{prefijo} --- {cabecera} (extraer_meridiano) ---\n"
