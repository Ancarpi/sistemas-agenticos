-- schema.sql --- Banco Meridiano, de una vez y reaplicable:
--   psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f schema.sql

CREATE EXTENSION IF NOT EXISTS vector;      -- pgvector 0.8.0+
CREATE SCHEMA IF NOT EXISTS meridiano;
SET search_path = meridiano, public;

-- La 7.4 creó la tabla en public. Va ANTES del CREATE: si dejas
-- las dos, escribes en una y lees de otra.
ALTER TABLE IF EXISTS public.manuales_meridiano
    SET SCHEMA meridiano;

-- Las tres primeras columnas las nombra PGVectorStore: o se las
-- renombras también a create_sync(), o falla al arrancar.
CREATE TABLE IF NOT EXISTS manuales_meridiano (
...
CREATE INDEX IF NOT EXISTS manuales_filtros
    ON manuales_meridiano (tipo, producto, version);
...
GRANT USAGE ON SCHEMA meridiano TO agente_lectura;
GRANT SELECT ON manuales_meridiano TO agente_lectura;
GRANT agente_lectura TO CURRENT_USER;   -- si no, nadie se lo pone

ALTER TABLE manuales_meridiano ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE manuales_meridiano FORCE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS manuales_ingesta ON manuales_meridiano;
CREATE POLICY manuales_ingesta ON manuales_meridiano
    FOR ALL TO CURRENT_USER USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS manuales_por_nivel ON manuales_meridiano;
CREATE POLICY manuales_por_nivel ON manuales_meridiano
    FOR SELECT USING (
        clasificacion = ANY (string_to_array(
            coalesce(current_setting('meridiano.niveles', true),
                     'publica'), ',')));
...
-- «Solo añadir» no es una promesa: es el FORCE del 7.6 otra vez,
...
CREATE ROLE meridiano_app LOGIN NOSUPERUSER NOBYPASSRLS;
GRANT INSERT, SELECT ON registro_ia TO meridiano_app;
