# servidor.py --- el transporte, y nada mas: el cerebro es el
...
from grafo_conciliacion import app as grafo  # 'app' ya esta cogido

api = FastAPI()
# asyncio solo guarda referencias DEBILES a sus tareas: sin este
# set, el recolector puede llevarse una a medio ejecutar.
VIVAS: set[asyncio.Task] = set()

# Art. 50, literal y copiable. Cambia el responsable y ya esta.
AVISO = ("Hablas con un asistente automatico de Banco Meridiano: "
         "no es una persona. Puede equivocarse, y no decide nada "
         "sobre tu dinero por si mismo. Escribe «quiero hablar "
         "con una persona» y te paso con el equipo.")
...
async def correr(hilo: str, turno: Turno, cola: asyncio.Queue):
    ...
    # caido, el core que no contesta --- y nadie pone el centinela,
    ...
    try:
        ...
        # nadie, asi que un raise aqui no aborta nada --- solo deja
...
async def chat(cliente_id: str, turno: Turno):
    # AQUI, y en ningun otro sitio, se decide la identidad de la
    # conversacion: un grafo, miles de clientes, un hilo cada uno.
    # OJO: tal cual, el cliente_id viene del path y cualquiera
    # escribe en el hilo de otro. En produccion sale del token de
    # sesion (Depends), NUNCA de la URL. Es el IDOR del M16.
    ...
    nuevo = not grafo.get_state(config(cliente_id)).values
    # create_task, no await: la tarea es DUEÑA del grafo y sobrevive
    # a la conexion. Con el grafo dentro del generador de abajo,
...
def ultimo(cliente_id: str):
    # El flujo no es la fuente de verdad; el checkpoint lo es (6.3).
    # Mismo IDOR que arriba, y aqui se LEE: el cliente_id sale del
    # token de sesion, no del path.
    estado = grafo.get_state(config(cliente_id))
...
# --- anadido del M17.2 (extraer_meridiano) ---
# Sondas de liveness y readiness con cache de 5 s. Esta si se pega al final, como dice el libro.
# se pega al final del servidor.py del 12.5: `asyncio` y `logging`
# ya están importados allí, y `grafo` y `config` también.
import os
import time
...
async def _sano() -> bool:
    ...
    try:
        ...
        # producción es el pool, y un SELECT 1 abre el suyo. A un
        # hilo, porque el grafo del 12.5 es SÍNCRONO.
        await asyncio.to_thread(grafo.get_state, config("sonda"))
...
# --- costura PENDIENTE del M17.2 (extraer_meridiano) ---
