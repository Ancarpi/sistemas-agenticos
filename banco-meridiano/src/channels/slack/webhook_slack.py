...
async def editar(canal: str, hilo_ts: str, ts, texto: str) -> str:
    if ts is None:                      # todavia no hay que editar
...
def referencia_de(texto: str) -> str:
    # EstadoBackoffice la exige (6.3). En web va en el JSON; aqui
    # se saca del mensaje, y si no viene el triaje la pedira.
...
async def entregar_en_slack(hilo: str, ev: dict):
    ...
    if "thread_ts" not in ev:
        # Art. 50: hilo nuevo, primer mensaje. Aqui no hay pie de
        # pagina donde esconderlo, y por eso vive en el flujo.
        await publicar(canal, hilo_ts, AVISO)
    # Sin token a token: la actividad EDITA un unico mensaje y la
...
async def eventos(req: Request, tareas: BackgroundTasks,
    ...
    # cuadra nunca y buscaras el fallo en la clave, que esta bien.
    ...
    # con un solo caracter raro lanzaria UnicodeEncodeError y daria
    ...
    # Sin estas dos lineas, Slack nunca habilita el endpoint.
    ...
    # se deduplica por event_id en tabla, no por cabecera: asi un
    # evento genuinamente perdido si se reentrega.
