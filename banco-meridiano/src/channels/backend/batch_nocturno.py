...
#   0 2 * * *  cd /srv/meridiano && uv run python batch_nocturno.py
import os
...
def cerrar(cur, ref: str, motivo: str) -> bool:
    ...
    cur.execute("UPDATE meridiano.incidencias SET estado='resuelta',"
...
def marcar(cur, ref: str, cola: str, motivo: str | None = None):
    cur.execute("UPDATE meridiano.incidencias SET estado='escalada',"
...
def main() -> None:
    ...
    with psycopg.connect(os.environ["DATABASE_URL"]) as bd, \
        ...
        cur.execute("SELECT referencia FROM meridiano.incidencias"
...
def informe(cur, n: dict, gasto: float) -> str:
    ...
    cur.execute("SELECT referencia, motivo FROM"
                " meridiano.incidencias WHERE cola='aprobacion'"
                " ORDER BY abierta_en")
    espera = cur.fetchall()
