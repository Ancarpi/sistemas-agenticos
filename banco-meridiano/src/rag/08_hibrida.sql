...
-- public: sin esta línea, manuales_meridiano a secas ni siquiera
-- resuelve, porque el 7.6 se la llevó con SET SCHEMA.
...
CREATE TEXT SEARCH CONFIGURATION es_meridiano (COPY = spanish);
ALTER TEXT SEARCH CONFIGURATION es_meridiano
...
ALTER TABLE manuales_meridiano
  ADD COLUMN tsv tsvector
  GENERATED ALWAYS AS (to_tsvector('es_meridiano', content)) STORED;

CREATE INDEX manuales_tsv_gin ON manuales_meridiano USING gin (tsv);
