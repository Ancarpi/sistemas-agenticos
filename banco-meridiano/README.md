...
Banco Meridiano es el caso único del libro: un banco con un back-office de incidencias SEPA, cinco superficies sobre un solo cerebro y un Postgres detrás. Aquí está el código de ese caso, en el árbol que el Ejercicio 0.1 impone, para que puedas leerlo, ejecutar lo que se ejecuta y comparar tu versión con la del libro.
...
| `src/core/politica.py` | La adenda del 35.2 es una llamada de ejemplo a nivel de módulo: hay que comentarla o moverla. Ver `COSTURAS.md`. |
| `src/core/memoria.py` | Las tres APIs del 34.5 son firmas con cuerpo `...`, y sus tres tipos de *receipt* están por escribir. |
...
- Se anuncia reaplicable y casi lo es: el `CREATE ROLE meridiano_app` del 16.6 no lleva `IF NOT EXISTS`, así que la segunda pasada falla en esa línea. El rodeo con `DO $do$` del 7.6 es el patrón correcto.
